...
export class ExcelService {
   ...
   *   - 稅額行（會計科目=進項稅額）：清空項目金額、發票金額、分攤參與部門、稅額等，只保留會計科目原幣金額（可開關見 sanitizeInheritedData 內註解）
  ...
  private static sanitizeInheritedData(
    ...
    if (accountName === '進項稅額') {
      ...
      // [可開關] 進項稅額行：清空「稅額」欄位，只保留「會計科目原幣金額」，避免同一張發票的稅額在主紀錄與進項稅額行重複出現（報表加總時重複計算）。
      // 若要恢復「稅額行也保留稅額」的原始行為，請將下一行註解掉。
      detailRow['稅額'] = '0';

      // 保留的欄位：會計科目原幣金額（稅額金額）。此欄位在 mergeDetailWithMaster 中已處理。
