 ...
 *
 * 背景：
 * COMMEET 同步/財務上傳時，[入帳對象] 可能包含較長的銀行資訊字串，
 * 原本 NVARCHAR(50) 容易觸發 SQL Server「String or binary data would be truncated」錯誤。
 */
export async function up(knex: Knex): Promise<void> {
  const tableName = 'ExpendForm';
  const columnName = '入帳對象';
  const targetLen = 500;
  ...
  const isAlreadyEnough = isMax || (Number.isFinite(maxLen) && maxLen >= targetLen);
