...
- `POST /api/bank-convert`: 國泰整批付款轉檔（Commeet 網銀 `.txt` 或付款資料 Excel `.xlsx`/`.xls` → 固定寬度 361 bytes + CRLF 之 `.txt`，Big5）。手動測試步驟見 `docs/BANK_CONVERT_TESTING.md`。
...
- **國泰整批付款轉檔** (`BankConverterService` + `server/constants/bankConverterConfig.ts` / `bankConverterExcelConfig.ts`): 將 Commeet 網銀匯出或「付款資料」工作表對應為國泰上傳格式；輸出每行 361 bytes，手續費 13/15 仍由 `HandlingFeeService.isSpecialCompany`（依 Excel `付款對象名稱`）決定。
